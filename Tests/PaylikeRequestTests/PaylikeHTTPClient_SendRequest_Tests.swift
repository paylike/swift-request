...
final class PaylikeHTTPClient_SendRequest_Tests: XCTestCase {
    ...
    func testGetRequest_completionHandler() throws {
        let swiftyServer = HttpServer()
        swiftyServer["/bar"] = { request in
            XCTAssertEqual(request.method, "GET")
            XCTAssertEqual(request.headers["accept-version"], "1")
    ...
    @available(swift, deprecated: 5.5)
    func testPostJSONData_completionHandler() throws {
    ...
    @available(swift, deprecated: 5.5)
    func testQuery_completionHandler() throws {
        let swiftyServer = HttpServer()
        swiftyServer["/bar"] = { request in
            XCTAssertEqual(request.queryParams.count, 1)
            let param = request.queryParams[0]
    ...
    @available(swift, deprecated: 5.5)
    func testForm_completionHandler() throws {
    ...
    @available(swift, deprecated: 5.5)
    func testResponseString_completionHandler() throws {
    ...
    @available(swift, deprecated: 5.5)
    func testTimeout_completionHandler() throws {
        let server = HttpServer()
        server["/bar"] = { request in
            Thread.sleep(forTimeInterval: 5)
            return .ok(.json(["message": "foo"]))
